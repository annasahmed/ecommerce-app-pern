...
const getGlobalSetting = async (req, res) => {
	try {
		...
		res.send(
			globalSetting?.setting || { message: "Global Settings Not Found" },
		);
	} catch (err) {
		res.status(500).send({
			message: err.message,
...
const getStoreSetting = async (req, res) => {
	try {
		...
		res.send(storeSetting?.setting || { message: "Store Settings Not Found" });
	} catch (err) {
		res.status(500).send({
			message: err.message,
		});
...
const getStoreCustomizationSetting = async (req, res) => {
	try {
		...
		if (!storeCustomizationSetting) {
			return res
				.status(400)
				.send({ message: "Store Customization Settings not found" });
		}
