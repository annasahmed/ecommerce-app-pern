...
const ReturnedDrawer = ({ id, data }) => {
	...
	const onSubmit = async (data) => {
		try {
			...
			if (id) {
				const res = await ReturnedServices.updateReturned(id, returnedData);
				setIsUpdate(true);
				...
				const res = await ReturnedServices.addReturned(returnedData);
				setIsUpdate(true);
				setIsSubmitting(false);
				notifySuccess(res.message);
				closeDrawer();
	...
	return (
		<>
			<DrawerHeader
				id={id}
				register={register}
				updateTitle={t("UpdateReturned")}
				updateDescription={t("UpdateReturnedDescription")}
				addTitle={t("AddReturnedTitle")}
				addDescription={t("AddReturnedDescription")}
			/>
			...
			<Scrollbars className="w-full md:w-7/12 lg:w-8/12 xl:w-8/12 relative dark:bg-customGray-700 dark:text-customGray-200">
				<form onSubmit={handleSubmit(onSubmit)}>
					...
					<DrawerButton id={id} title="Returned" isSubmitting={isSubmitting} />
				</form>
			</Scrollbars>
		</>
	);
